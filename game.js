...
function jump() {
  ...
  if (d.onGround || d.jumpCount < 2) {
    ...
    beep(d.jumpCount === 1 ? 520 : 720, .05, "square", .02);
...
function spawnObstacle() {
  const r = Math.random();
  if (r < .2 && game.score > 500) {
    game.obstacles.push({ x: W + 40, y: groundY - 120, w: 60, h: 34, type: "drone" });
  } else {
    const h = 42 + Math.random() * 62;
    const w = 32 + Math.random() * 34;
    game.obstacles.push({ x: W + 40, y: groundY - h, w, h, type: r > .55 ? "rock" : "cone" });
  }
}
...
function spawnCoinLine() {
  ...
  for (let i = 0; i < 5; i++) {
    game.coinsList.push({ x: W + i * 38, y: baseY + Math.sin(i) * 18, r: 12, taken: false });
  }
}

function spawnPowerup() {
  const type = Math.random() > .5 ? "shield" : "magnet";
  game.powerups.push({ x: W + 40, y: groundY - 150 - Math.random() * 80, w: 34, h: 34, type });
}

function spawnBoss() {
  game.boss = { x: W + 80, y: 95, hp: 3, cooldown: 50, active: true };
  say("警告：ボスドローン接近！");
  beep(110, .25, "sawtooth", .035);
}
...
function playerBox() {
  const d = game.dino;
  return { x: d.x + 28, y: d.y - d.h + 20, w: d.w - 45, h: d.h - 28 };
}
...
function update() {
  ...
  if (game.frame % Math.max(42, Math.floor(105 - game.speed * 4)) === 0) spawnObstacle();
  if (game.frame % 150 === 0) spawnCoinLine();
  if (game.frame % 520 === 0) spawnPowerup();
  if (game.score > 1400 && !game.boss && Math.floor(game.score) % 1500 < 3) spawnBoss();

  for (const o of game.obstacles) o.x -= moveSpeed;
  ...
  for (const p of game.powerups) p.x -= moveSpeed;
  game.powerups = game.powerups.filter(p => p.x > -50);

  for (const l of game.lasers) l.x -= moveSpeed + 3;
  game.lasers = game.lasers.filter(l => l.x + l.w > -40);

  for (const p of game.particles) {
    p.x += p.vx - moveSpeed * .25;
    p.y += p.vy;
    p.vy += .25;
    p.life--;
  }
  ...
  if (game.boss) {
    ...
    if (game.boss.cooldown <= 0) {
      game.lasers.push({ x: game.boss.x, y: game.boss.y + 28, w: 80, h: 10 });
      game.boss.cooldown = Math.max(32, 80 - game.speed * 3);
      say("レーザー回避！");
    }
    if (game.boss.x < -120) game.boss = null;
  }

  const box = playerBox();
  for (const o of game.obstacles) if (rectsHit(box, o)) hitDamage();
  for (const l of game.lasers) if (rectsHit(box, l)) hitDamage();
  ...
  for (const c of game.coinsList) {
    if (circleHitRect(c, box)) {
      ...
      beep(760 + Math.min(game.combo, 12) * 28, .04, "triangle", .018);
  ...
  if (Math.floor(game.score) % 700 === 0) say("AI難易度、自動上昇中！");
}
...
function drawBackground() {
  ...
  sky.addColorStop(.58,"#e0f9ff");
  sky.addColorStop(1,"#0f2b42");
  ctx.fillStyle = sky;
  ctx.fillRect(0,0,W,H);

  ctx.fillStyle = "rgba(0, 220, 255, .16)";
  for (let i = 0; i < 9; i++) {
    const x = (i * 150 - (game.frame * game.speed * .22) % 150);
    ctx.beginPath();
    ctx.arc(x, 95 + Math.sin((game.frame+i*20)/40)*18, 30, 0, Math.PI*2);
    ctx.fill();
  }

  ctx.strokeStyle = "rgba(0, 130, 180, .25)";
  for (let x = -120; x < W + 120; x += 120) {
    const sx = x - (game.frame * game.speed * .5) % 120;
    ctx.beginPath();
    ctx.moveTo(sx, 260); ctx.lineTo(sx + 60, 210); ctx.lineTo(sx + 120, 260);
    ctx.stroke();
  }

  ctx.fillStyle = "#143248";
  ctx.fillRect(0, groundY, W, H-groundY);
  ctx.strokeStyle = "rgba(0, 238, 255, .7)";
  ctx.lineWidth = 3;
  ctx.beginPath();
  ctx.moveTo(0, groundY); ctx.lineTo(W, groundY); ctx.stroke();

  ctx.strokeStyle = "rgba(0, 238, 255, .25)";
  for (let x = -80; x < W + 80; x += 80) {
    const sx = x - (game.frame * game.speed) % 80;
    ctx.beginPath(); ctx.moveTo(sx, groundY + 36); ctx.lineTo(sx + 42, groundY + 36); ctx.stroke();
  }
}

function drawDino() {
  const d = game.dino;
  const runBob = d.onGround ? Math.sin(game.frame * .32) * 5 : 0;
  const tilt = d.onGround ? Math.sin(game.frame * .22) * .03 : d.vy * .012;
  const scaleY = d.onGround ? 1 + Math.sin(game.frame * .32) * .025 : 1;
  ...
  ctx.globalAlpha = d.invincible > 0 && game.frame % 6 < 3 ? .45 : 1;
...
function drawObstacles() {
  for (const o of game.obstacles) {
    ctx.save();
    if (o.type === "rock") {
      ctx.fillStyle = "#263849"; ctx.strokeStyle = "#00dfff"; ctx.lineWidth = 2;
      ctx.beginPath(); ctx.roundRect(o.x, o.y, o.w, o.h, 8); ctx.fill(); ctx.stroke();
    } else if (o.type === "drone") {
      ctx.fillStyle = "#111a2b"; ctx.strokeStyle = "#ff4df0"; ctx.lineWidth = 3;
      ctx.beginPath(); ctx.roundRect(o.x, o.y, o.w, o.h, 12); ctx.fill(); ctx.stroke();
      ctx.fillStyle = "#ff4df0"; ctx.beginPath(); ctx.arc(o.x + 15, o.y + 17, 5, 0, Math.PI*2); ctx.arc(o.x + 45, o.y + 17, 5, 0, Math.PI*2); ctx.fill();
    } else {
...
function drawCoinsPowerupsBoss() {
  for (const c of game.coinsList) {
    ...
    ctx.rotate(game.frame * .08);
    ctx.fillStyle = "#ffd84d";
    ctx.strokeStyle = "#fff7b0";
    ctx.lineWidth = 3;
    ctx.beginPath(); ctx.ellipse(0, 0, 12, 16, 0, 0, Math.PI*2); ctx.fill(); ctx.stroke();
    ctx.fillStyle = "#9b6b00"; ctx.font = "bold 14px system-ui"; ctx.textAlign = "center"; ctx.fillText("AI", 0, 5);
    ctx.restore();
  }
  for (const p of game.powerups) {
    ctx.save();
    ...
    ctx.fillStyle = "#062130"; ctx.font = "bold 20px system-ui"; ctx.textAlign = "center";
  ...
  if (game.boss) {
    const b = game.boss;
    ctx.save();
    ctx.fillStyle = "#151528"; ctx.strokeStyle = "#ff45e6"; ctx.lineWidth = 4;
    ctx.beginPath(); ctx.roundRect(b.x, b.y, 110, 62, 18); ctx.fill(); ctx.stroke();
    ctx.fillStyle = "#ff45e6"; ctx.beginPath(); ctx.arc(b.x + 35, b.y + 32, 8, 0, Math.PI*2); ctx.arc(b.x + 75, b.y + 32, 8, 0, Math.PI*2); ctx.fill();
    ctx.restore();
  }
  for (const l of game.lasers) {
    ctx.save();
    ctx.fillStyle = "rgba(255, 30, 180, .85)";
    ctx.shadowColor = "#ff2edb"; ctx.shadowBlur = 18;
...
function drawParticles() {
  for (const p of game.particles) {
    ...
    ctx.fillStyle = p.color === "coin" ? "#ffd84d" : p.color === "shield" ? "#00ff99" : "rgba(0, 230, 255, .8)";
...
function drawUI() {
  ...
  ctx.beginPath(); ctx.roundRect(18, 16, 300, 122, 18); ctx.fill();
  ...
  if (game.messageTimer > 0) {
    ...
    ctx.beginPath(); ctx.roundRect(340, 20, 465, 60, 16); ctx.fill();
  ...
  if (game.paused) {
    ...
    ctx.fillStyle = "#fff"; ctx.textAlign = "center"; ctx.font = "bold 52px system-ui";
  ...
  if (game.over) {
    ...
    ctx.font = "bold 56px system-ui"; ctx.fillText("GAME OVER", W/2, H/2 - 45);
    ctx.font = "bold 26px system-ui"; ctx.fillText(`FINAL SCORE ${finalScore}`, W/2, H/2 + 0);
    ctx.font = "20px system-ui"; ctx.fillText("START / RESTARTで再挑戦", W/2, H/2 + 38);
    ctx.textAlign = "left";
  }
}
...
window.addEventListener("keydown", e => {
  ...
  if (["ShiftLeft", "ShiftRight", "ArrowDown", "KeyS"].includes(e.code)) { e.preventDefault(); dash(); }
